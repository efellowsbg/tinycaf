...
locals {
  ...
  subscription_scope_assignments = {
    ...
    if contains(try(jsondecode(file("${var.assignments_folder}/${v}"))["properties"]["scope"], ""), "current_scope_resource_id") && !contains(try(jsondecode(file("${var.assignments_folder}/${v}"))["properties"]["scope"], ""), "/")
  }
