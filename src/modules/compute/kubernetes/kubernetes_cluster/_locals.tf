locals {
  ...
  for ref in var.settings.identity.managed_identity_ref : (
    var.resources.managed_identities[ref] != null ? var.resources.managed_identities[ref].id : null
  )
  if var.resources.managed_identities[ref] != null
]
  tags = merge(
