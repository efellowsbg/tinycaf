locals {
  ...
  subnet_id = try(
    var.resources.virtual_networks[split("/", var.settings.site_config.ip_restriction.subnet_ref)[0]].subnets[split("/", var.settings.site_config.ip_restriction.subnet_ref)[1]].id,
    null
  )
}
