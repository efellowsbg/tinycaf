resource "azurerm_storage_account" "main" {
  ...
  network_rules {
    ...
    virtual_network_subnet_ids = try(var.settings.network, null) == null ? null : [
        for key, value in var.settings.network : can(value.subnet_id) ? value.subnet_id : var.resources.virtual_networkstry[value.vnet_ref].subnets[value.subnet_ref].id
      ]
    }
