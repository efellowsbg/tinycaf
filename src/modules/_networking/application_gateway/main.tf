resource "azurerm_application_gateway" "main" {
  ...
  dynamic "waf_configuration" {
    for_each = can(var.settings.waf_configuration) ? [1] : []
    content {
      ...
      rule_set_type            = try(var.settings.waf_configuration.rule_set_type, null)
      file_upload_limit_mb     = try(var.settings.waf_configuration.file_upload_limit_mb, null)
      request_body_check       = try(var.settings.waf_configuration.request_body_check, null)
      max_request_body_size_kb = try(var.settings.waf_configuration.max_request_body_size_kb, null)
