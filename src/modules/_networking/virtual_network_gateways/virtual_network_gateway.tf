resource "azurerm_virtual_network_gateway" "main" {
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      subnet_id = can(ip_configuration.value.subnet_id) ? ip_configuration.value.subnet_id : module.virtual_networks[ip_configuration.value.vnet_ref].subnet_ids["${ip_configuration.value.vnet_ref}.${ip_configuration.value.subnet_ref}"]
    }
  }
}
